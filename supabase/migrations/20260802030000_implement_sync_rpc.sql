-- Tarea 8.2: primitivas remotas transaccionales para sincronización local-first.
-- Esta migración se mantiene local hasta que se aplique explícitamente con Supabase CLI.
...
as $$
begin
  new.updated_at := greatest(old.updated_at, coalesce(new.updated_at, now()), now());
  return new;
end;
$$;
...
begin
  ...
  if p_operation_type not in ('create', 'update', 'delete', 'pay_recurring_occurrence') then
    raise exception 'unsupported_operation_type: %', p_operation_type using errcode = '22023';
  end if;
  ...
  if v_inserted = 0 then
    if not exists (
      ...
      raise exception 'operation_id_belongs_to_another_user' using errcode = '42501';
    end if;
  ...
  if p_operation_type = 'pay_recurring_occurrence' then
    if p_entity_type <> 'recurringPaymentOccurrence' then
      raise exception 'compound_operation_requires_occurrence_entity' using errcode = '22023';
    end if;
    ...
    v_incoming_updated_at := (p_payload #>> '{occurrence,updated_at}')::timestamptz;
    select updated_at
    ...
    if v_existing_updated_at is not null and v_existing_updated_at >= v_incoming_updated_at then
      return jsonb_build_object(
        'status', 'remote_wins',
    ...
    if v_related_updated_at is not null
      and v_related_updated_at >= (p_payload #>> '{expense,updated_at}')::timestamptz then
      return jsonb_build_object(
        'status', 'remote_wins',
        'entity_updated_at', v_existing_updated_at,
    ...
    select (jsonb_populate_record(null::public.recurring_payment_occurrences, v_payload)).*
    ...
    v_incoming_updated_at := (p_payload #>> '{expense,updated_at}')::timestamptz;
    v_payload := (p_payload -> 'expense') || jsonb_build_object(
      'user_id', v_user_id,
      'updated_at', greatest(v_incoming_updated_at, clock_timestamp())
    );
    insert into public.expenses
    select (jsonb_populate_record(null::public.expenses, v_payload)).*
    on conflict (id) do update set
      period_id = excluded.period_id,
  ...
  case p_entity_type
    ...
    when 'income' then
      ...
      v_update_clause := 'period_id = excluded.period_id, amount = excluded.amount, description = excluded.description, date = excluded.date, updated_at = excluded.updated_at, deleted_at = excluded.deleted_at';
    when 'expense' then
      v_table_name := 'expenses';
      v_update_clause := 'period_id = excluded.period_id, category_id = excluded.category_id, amount = excluded.amount, description = excluded.description, date = excluded.date, recurring_occurrence_id = excluded.recurring_occurrence_id, updated_at = excluded.updated_at, deleted_at = excluded.deleted_at';
    when 'category' then
    ...
    when 'recurringPaymentOccurrence' then
      ...
      v_update_clause := 'recurring_payment_id = excluded.recurring_payment_id, period_id = excluded.period_id, due_date = excluded.due_date, status = excluded.status, updated_at = excluded.updated_at, deleted_at = excluded.deleted_at';
...
revoke all on function public.apply_sync_operation(uuid, text, uuid, text, jsonb)
  from public, anon, authenticated;
grant execute on function public.apply_sync_operation(uuid, text, uuid, text, jsonb)
  to authenticated;
