...
beforeEach(async () => {
  // Manual entry uses today's date; keep it inside the fixed August fixture.
  // setSystemTime mocks Date without replacing IndexedDB/user-event timers.
...
describe.each(['system', 'light', 'dark'] as const)(
  'Receipt local-first integration (%s)',
  (theme) => {
    beforeEach(() => {
    ...
    it('confirma una vez: persiste un Expense y una SyncOperation sin imagen ni red', async () => {
      ...
      await user.click(
        screen.getByRole('button', { name: 'Elegir de galería' }),
      )
      await user.click(
    ...
    it('un fallo local no muestra éxito ni crea cola parcial', async () => {
      ...
      expect(
        screen.queryByText('Gasto guardado en este dispositivo.'),
      ).toBeNull()
      expect(await db.expenses.count()).toBe(0)
      expect(await db.syncOperations.count()).toBe(0)
      expect(screen.getByLabelText('Descripción')).toHaveValue('Prueba')
      expect(screen.getByLabelText('Monto (MXN)')).toHaveValue('10.00')
      expect(screen.getByLabelText('Fecha')).toHaveValue('2026-08-02')
      expect(screen.getByLabelText('Categoría')).toHaveValue(categoryId)
    })

    it('guest guarda manualmente sin invocar OCR ni crear operación remota', async () => {
      const recognizeReceipt = vi.fn()
      const guestOwner = 'guest:receipt-owner'
      await db.periods.update(periodId, { ownerId: guestOwner })
      await db.categories.update(categoryId, { ownerId: guestOwner })
      const useCase = new CreateExpense(
        new DexieExpenseRepository(db, guestOwner),
        new DexiePeriodRepository(db, guestOwner),
        new DexieCategoryRepository(db, guestOwner),
        { generate: () => expenseId },
        { now: () => now },
      )
      const createExpense = vi.spyOn(useCase, 'execute')
      render(
        <ReceiptCaptureFlow
