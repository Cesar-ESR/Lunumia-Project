...
import { amountCentsArbitrary } from '@infrastructure/sync/property/arbitraries'
...
beforeEach(() => {
  database = new GastoClaroDB(`recurring-property-${crypto.randomUUID()}`)
})
...
describe('propiedades del pago compuesto recurrente', () => {
  it('PBT: crea gasto + paga ocurrencia + encola exactamente una operación compuesta', async () => {
    await fc.assert(
      fc.asyncProperty(amountCentsArbitrary, async (amount) => {
